 ...
 */
package ca.mcgill.sable.clara.fianalysis;
...
public class PathInfoFinder {
	...
	/**
	 * A predicate over a TracePattern state machine state.
	 * @author Eric Bodden
	 */
	 ...
	 * @param TracePattern a TracePattern state machine.
	 */
	public PathInfoFinder(TracePattern TracePattern) {
		this(TracePattern, new DEFAULT_PREDICATE());
	}
	 ...
	 */
	public PathInfoFinder(TracePattern TracePattern, StatePredicate pred) {
		this(TracePattern,pred,TracePattern.getStateMachine().getInitialStates());
	}
	 ...
	 */
	public PathInfoFinder(TracePattern TracePattern, Set<? extends State> initialStates) {
		this(TracePattern,new DEFAULT_PREDICATE(),initialStates);
	}
	...
	public PathInfoFinder(TracePattern TracePattern, StatePredicate pred, Set<? extends State> initialStates) {
		pathInfos = new HashSet<PathInfo>();
		
		StateMachine sm = TracePattern.getStateMachine();
	 ...
	 * @param sm any TracePattern state machine
	...
	/**
	 * @return all path infos computed for the given TracePattern
	 */
